...
- An **Example todos** page in the left sidebar that adds, completes, and
  removes todos.
- A `bb typesafe-router` command that does the same from a terminal.
...
The bundled skill tells an agent to read the list with `bb typesafe-router list`, add
one todo at a time with `bb typesafe-router add`, and close finished work with
`bb typesafe-router done`.
